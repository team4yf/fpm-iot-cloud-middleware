...
describe('Common Protocol Test', function(){
  ...
  it('0x11 Version', function(done){
    // 11 00 00 03 00 00 00 03 ff fe fd fc 05 00 00 01 00 00 53 92
    const origin = data;
    const packet = decoder(origin);
    //
    const { header, payload } = packet;
    //vid: 0, uid, pid, sid, fn, extra
    const { vid, uid, pid, sid, fn, extra } = header;
    assert.strictEqual(vid, 0x11, 'VID should be 0x11');
  ...
  it('payload test', function(done){
    ...
    const origin = `${ '11 00 00 03 00 00 00 03'.trim()}${payloadBuf.toString('hex')}`;
    const packet = decoder(origin);
